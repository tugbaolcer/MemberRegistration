...
namespace DevFramework.Core.CrossCuttingConcernes.Security.Web
{
    public class AuthenticationHelper
    {
        public static void CreateAuthCookie(Guid id, string userName, string email, DateTime expiration, string[] roles, bool rememberMe, string firstName, string lastName)
        {
            ...
            string encTicket = FormsAuthentication.Encrypt(authTicket);//Şifreli kimlik doğrulama 
            HttpContext.Current.Response.Cookies.Add(new HttpCookie(FormsAuthentication.FormsCookieName, encTicket));//şifrelenmiş etiketi cookiye bastık.
        }

        private static string CreateAuthTags(string email, string[] roles, string firstName, string lastName, Guid id)//Formatlı bir string oluşturmak için oluşturuyoruz.
        {
